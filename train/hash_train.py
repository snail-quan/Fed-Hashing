...
class Trainer(TrainBase):
    ...
    def __init__(self, index_file, caption_file, label_file, rank=0):
        args = get_args()
    ...
    def _init_dataset(self):
        ...
        self.args.index_file = os.path.join("./dataset", self.args.dataset, self.index_file)
        self.args.caption_file = os.path.join("./dataset", self.args.dataset, self.caption_file)
        self.args.label_file = os.path.join("./dataset", self.args.dataset, self.label_file)
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            if self.args.hash_layer == "select":
                hash_img = torch.cat(hash_img, dim=-1) if isinstance(hash_img, list) else hash_img.view(
                    hash_img.shape[0], -1)
                hash_text = torch.cat(hash_text, dim=-1) if isinstance(hash_text, list) else hash_text.view(
                    hash_text.shape[0], -1)
    ...
    def similarity_loss(self, a: torch.Tensor, b: torch.Tensor, label_sim: torch.Tensor, threshold=0.05):
        ...
        similarity = (
                    1 - cosine_similarity(a, b)) if self.args.similarity_function == "cosine" else euclidean_similarity(
            a, b)
        ...
        if self.args.similarity_function == "cosine":
            ...
            negative_similarity = torch.tensor([1.]).expand_as(negative_similarity).to(self.rank) * (
                        1 - label_sim) - negative_similarity
            ...
            negative_similarity = torch.tensor([max_value]).expand_as(negative_similarity).to(self.rank) * (
                        1 - label_sim) - negative_similarity
    ...
    def our_loss(self, image, text, label, epoch, times):
        ...
        intra_similarity_loss = (
                    intra_positive_loss + intra_negative_loss) if self.args.similarity_function == "euclidean" else (
                    intra_positive_loss + intra_negative_loss)
        inter_similarity_loss = inter_positive_loss_t + inter_positive_loss_i + (
                    inter_negative_loss_i + inter_negative_loss_t) if self.args.similarity_function == "euclidean" else inter_positive_loss_t + inter_positive_loss_i + inter_negative_loss_i + inter_negative_loss_t
        ...
        if self.args.hash_layer != "select":
            ...
            if self.global_step % self.args.display_step == 0:
                self.logger.info(
                    f">>>>>> Display >>>>>> [{epoch}/{self.args.epochs}], [{times}/{len(self.train_loader)}]: all loss: {loss.data}, " \
    ...
    def valid(self, epoch):
        ...
        self.change_state(mode="valid")
        query_img, query_txt = self.get_code(self.query_loader,
                                             self.args.query_num) if self.args.hash_layer == "select" else super().get_code(
            self.query_loader, self.args.query_num)
        retrieval_img, retrieval_txt = self.get_code(self.retrieval_loader,
                                                     self.args.retrieval_num) if self.args.hash_layer == "select" else super().get_code(
